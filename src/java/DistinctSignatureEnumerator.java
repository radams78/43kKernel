...
import java.util.*;
    ...
    /**
     * Constructor processes boundaryFile through readBoundaries() producing a list of boundaries and a list of strings.
     * A {@link BoundaryGarden} myGarden is then created from these two lists.
     * Then run new {@link BasicRegionGenerator} for each member of myGarden.
     */	
public class DistinctSignatureEnumerator {
	...
	DistinctSignatureEnumerator(String boundaryFile) throws Exception {
		...
		for(int i = 0; i < myGarden.n; ++i) {basicRegionsByBoundary.add(BasicRegionGenerator.makeBasicRegionGenerator(myGarden, i));}
		System.out.println("done.");
